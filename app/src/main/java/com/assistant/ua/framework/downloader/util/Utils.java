package com.assistant.ua.framework.downloader.util;
...
public class Utils {
    ...
    public static void deleteFile(File file) {
        boolean result = file != null && file.exists() && file.delete();
    ...
    public static synchronized File createFile(String path, String name) {
        ...
        if (!parentFile.exists()) {
            Log.d("", "result = " + parentFile.mkdir());
        }

        File file = new File(parentFile, name);
        if (!file.exists()) {
            try {
                Log.d("", "result = " + file.createNewFile());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
