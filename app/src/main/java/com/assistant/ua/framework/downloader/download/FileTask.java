...
import com.assistant.ua.framework.downloader.data.Ranges;
...
public class FileTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            DownloadData data = Db.getInstance().getData(url);
            if (Utils.isFileExists(saveFile) && Utils.isFileExists(tempFile) && data != null && data.getStatus() != PROGRESS) {
                ...
                if (response != null && response.isSuccessful()) {
                    for (int i = 0; i < response.headers().size(); i++) {
                        ...
                        System.out.println("name = " + name + "; value = " + response.headers().get(name));
                    }
    ...
    private void saveRangeFile() {
        ...
        Db.getInstance().updateProgress(0, 0, PROGRESS, url);
        ...
        for (int i = 0; i < childTaskCount; i++) {
            ...
            Call call = OkHttpManager.getInstance().initRequest(url, range.start[i], range.end[i], new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    for (int i = 0; i < response.headers().size(); i++) {
                        String name = response.headers().name(i);
                        System.out.println("save file: name = " + name + "; value = " + response.headers().get(name));
                    }
